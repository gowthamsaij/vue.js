...
<div class="card_data align-items-center ">
    ...
    <b-card style="max-width: 25rem;" class="mb-2 shadow p-3 mb-5 bg-white rounded">
        ...
        <hr> <h3 style="text-align: center;">Merchant Login Panel</h3> <hr>
...
export default {
    ...
    methods: {
        onSubmit(evt) {
            ...
            this.axios({
                method: 'post',
                url: 'http://localhost/webcash/api/v2/merchants/token.json?hash_key=28a196fd38af3065ba85344dc3b1e171fa45cf34&skip=true',
                data: bodyFormData
                })
                .then( (response) =>  {
                    // console.log(response.data['data'].merchantName)
                    //handle success
                    if (response.data.success == true) {
                        const token = response.data['data'].token
                        const merchant_id = response.data['data'].source_merchant_id
                        const merchantName = response.data['data'].merchantName
                        const user = 'merchant'
